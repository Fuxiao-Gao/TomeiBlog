<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="usersList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="profile_pic" align="center" prop="profilePic" width="100">
        <template slot-scope="scope">
          <image-preview :src="scope.row.profilePic" :width="50" :height="50"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="profile_pic" prop="profilePic">
          <image-upload v-model="form.profilePic"/>
        </el-form-item>
        <el-form-item label="email" prop="email">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        profilePic: [
          { required: true, message: "profile_pic不能为空", trigger: "blur" }
        ],
        email: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除users编号为"' + ids + '"的数据项？').then(function() {
        return delUsers(ids);
      }).then(() => {
        this.getList();
